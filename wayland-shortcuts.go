...
func main() {
	...
	// Open real keyboard
	device_path, err := filepath.Glob("/dev/input/by-path/*-event-kbd")
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
	...
	device, err := ev.Open(device_path[0])
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
		...
		for _, event := range events {
			...
			if event.Type == ev.EV_KEY && event.Value != 2 {
				...
				if (len(k) == 2 && Super(k[0]) && Tab(k[1])) ||
					...
					if kPressed[ev.KEY_LEFTMETA] {
						kbDown(ev.KEY_LEFTMETA)
					}
					if kPressed[ev.KEY_RIGHTMETA] {
						kbDown(ev.KEY_RIGHTMETA)
					}
					...
					sAltTextKeys = false
					kbUp(ev.KEY_LEFTCTRL)
					if kPressed[ev.KEY_LEFTALT] {
						kbDown(ev.KEY_LEFTALT)
					}
					if kPressed[ev.KEY_RIGHTALT] {
						kbDown(ev.KEY_RIGHTALT)
					}
					...
					sCtrlLeft = false
					kbUp(ev.KEY_HOME)
					if kPressed[ev.KEY_LEFTCTRL] {
						kbDown(ev.KEY_LEFTCTRL)
					}
					if kPressed[ev.KEY_RIGHTCTRL] {
						kbDown(ev.KEY_RIGHTCTRL)
					}
					if kPressed[ev.KEY_LEFT] {
						kbDown(ev.KEY_LEFT)
					}
